...
class NetatmoThermostat {
  constructor(log, config) {
    ...
    this.refreshThermostat((error) => {
      if (error) {
        this.log('Error loading thermostat data');
      } else {
  ...
  refreshThermostat(callback) {
    this.log('Loading thermostat data from server');
  ...
  getServices() {
    ...
    informationService
      .setCharacteristic(Characteristic.Manufacturer, 'Vaillant')
      .setCharacteristic(Characteristic.Model, 'vSMART Thermostat');
